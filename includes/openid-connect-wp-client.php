...
/**
 * OpenID_Connect_WP_Client class.
 *
 * Plugin OIDC/oAuth client class.
 *
 * @package  OpenID_Connect_WP
 * @category Authentication
 */
class OpenID_Connect_WP_Client {

	/**
	 * The OIDC/oAuth client ID.
	 *
	 * @see OpenID_Connect_WP_Option_Settings::client_id
	 *
	 * @var string
	 */
	private $client_id;
	 ...
	 *
	 * @see OpenID_Connect_WP_Option_Settings::client_secret
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_WP_Option_Settings::scope
	 *
	 * @var string
	 */
	private $scope;
	 ...
	 *
	 * @see OpenID_Connect_WP_Option_Settings::endpoint_login
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_WP_Option_Settings::endpoint_userinfo
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_WP_Option_Settings::endpoint_token
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_WP_Option_Settings::redirect_uri
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_WP_Option_Settings::acr_values
	 *
	 * @var string
	 */
	private $acr_values;
	 ...
	 *
	 * @see OpenID_Connect_WP_Option_Settings::state_time_limit
	 *
	 * @var int
	 */
	 ...
	 * Client constructor.
	 *
	 * @param string                          $client_id         @see OpenID_Connect_WP_Option_Settings::client_id for description.
	 * @param string                          $client_secret     @see OpenID_Connect_WP_Option_Settings::client_secret for description.
	 * @param string                          $scope             @see OpenID_Connect_WP_Option_Settings::scope for description.
	 * @param string                          $endpoint_login    @see OpenID_Connect_WP_Option_Settings::endpoint_login for description.
	 * @param string                          $endpoint_userinfo @see OpenID_Connect_WP_Option_Settings::endpoint_userinfo for description.
	 * @param string                          $endpoint_token    @see OpenID_Connect_WP_Option_Settings::endpoint_token for description.
	 * @param string                          $redirect_uri      @see OpenID_Connect_WP_Option_Settings::redirect_uri for description.
	 * @param string                          $acr_values        @see OpenID_Connect_WP_Option_Settings::acr_values for description.
	 * @param int                             $state_time_limit  @see OpenID_Connect_WP_Option_Settings::state_time_limit for description.
	 * @param OpenID_Connect_WP_Option_Logger $logger            The plugin logging object instance.
	 */
	public function __construct( string $client_id, string $client_secret, string $scope, string $endpoint_login, string $endpoint_userinfo, string $endpoint_token, string $redirect_uri, string $acr_values, int $state_time_limit, OpenID_Connect_WP_Option_Logger $logger ) {
	...
	public function validate_authentication_request( $request ) {
		...
		if ( ! isset( $request['state'] ) ) {
			do_action( 'openid-connect-wp-no-state-provided' );
			return new \WP_Error( 'missing-state', __( 'Missing state.', 'openid-connect-wp' ), $request );
		}

		if ( ! $this->check_state( $request['state'] ) ) {
			return new \WP_Error( 'invalid-state', __( 'Invalid state.', 'openid-connect-wp' ), $request );
		}
	...
	public function get_authentication_code( $request ) {
		if ( ! isset( $request['code'] ) ) {
			return new \WP_Error( 'missing-authentication-code', __( 'Missing authentication code.', 'openid-connect-wp' ), $request );
		}
	...
	public function request_authentication_token( $code ) {
		...
		$request = apply_filters( 'openid-connect-wp-alter-request', $request, 'get-authentication-token' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'request_authentication_token', __( 'Request for authentication token failed.', 'openid-connect-wp' ) );
		}
	...
	public function request_new_tokens( $refresh_token ) {
		...
		// Allow modifications to the request.
		$request = apply_filters( 'openid-connect-wp-alter-request', $request, 'refresh-token' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'refresh_token', __( 'Refresh token failed.', 'openid-connect-wp' ) );
		}
	...
	public function get_token_response( $token_result ) {
		if ( ! isset( $token_result['body'] ) ) {
			return new \WP_Error( 'missing-token-body', __( 'Missing token body.', 'openid-connect-wp' ), $token_result );
		}
		...
		if ( is_null( $token_response ) ) {
			return new \WP_Error( 'invalid-token', __( 'Invalid token.', 'openid-connect-wp' ), $token_result );
		}
	...
	public function request_userinfo( $access_token ) {
		// Allow modifications to the request.
		$request = apply_filters( 'openid-connect-wp-alter-request', array(), 'get-userinfo' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'request_userinfo', __( 'Request for userinfo failed.', 'openid-connect-wp' ) );
		}
	...
	public function check_state( $state ) {
		...
		if ( ! get_option( '_transient_openid-connect-wp-state--' . $state ) ) {
			do_action( 'openid-connect-wp-state-not-found', $state );
			$state_found = false;
		}

		$valid = get_transient( 'openid-connect-wp-state--' . $state );

		if ( ! $valid && $state_found ) {
			do_action( 'openid-connect-wp-state-expired', $state );
		}
	...
	public function get_authentication_state( $request ) {
		if ( ! isset( $request['state'] ) ) {
			return new \WP_Error( 'missing-authentication-state', __( 'Missing authentication state.', 'openid-connect-wp' ), $request );
		}
	...
	public function get_id_token_claim( $token_response ) {
		...
		if ( ! isset( $token_response['id_token'] ) ) {
			return new \WP_Error( 'no-identity-token', __( 'No identity token.', 'openid-connect-wp' ), $token_response );
		}
		...
		if ( ! isset( $tmp[1] ) ) {
			return new \WP_Error( 'missing-identity-token', __( 'Missing identity token.', 'openid-connect-wp' ), $token_response );
		}
	...
	public function validate_id_token_claim( $id_token_claim ) {
		if ( ! is_array( $id_token_claim ) ) {
			return new \WP_Error( 'bad-id-token-claim', __( 'Bad ID token claim.', 'openid-connect-wp' ), $id_token_claim );
		}

		// Validate the identification data and it's value.
		if ( ! isset( $id_token_claim['sub'] ) || empty( $id_token_claim['sub'] ) ) {
			return new \WP_Error( 'no-subject-identity', __( 'No subject identity.', 'openid-connect-wp' ), $id_token_claim );
		}
		...
		if ( ! empty( $this->acr_values ) && isset( $id_token_claim['acr'] ) ) {
			if ( $this->acr_values != $id_token_claim['acr'] ) {
				return new \WP_Error( 'no-match-acr', __( 'No matching acr values.', 'openid-connect-wp' ), $id_token_claim );
			}
		}
	...
	public function get_user_claim( $token_response ) {
		...
		if ( is_wp_error( $user_claim_result ) || ! isset( $user_claim_result['body'] ) ) {
			return new \WP_Error( 'bad-claim', __( 'Bad user claim.', 'openid-connect-wp' ), $user_claim_result );
		}
	...
	public function validate_user_claim( $user_claim, $id_token_claim ) {
		...
		if ( ! is_array( $user_claim ) ) {
			return new \WP_Error( 'invalid-user-claim', __( 'Invalid user claim.', 'openid-connect-wp' ), $user_claim );
		}

		// Allow for errors from the IDP.
		if ( isset( $user_claim['error'] ) ) {
			$message = __( 'Error from the IDP.', 'openid-connect-wp' );
		...
		if ( $id_token_claim['sub'] !== $user_claim['sub'] ) {
			return new \WP_Error( 'incorrect-user-claim', __( 'Incorrect user claim.', 'openid-connect-wp' ), func_get_args() );
		}

		// Allow for other plugins to alter the login success.
		$login_user = apply_filters( 'openid-connect-wp-user-login-test', true, $user_claim );

		if ( ! $login_user ) {
			return new \WP_Error( 'unauthorized', __( 'Unauthorized access.', 'openid-connect-wp' ), $login_user );
		}
